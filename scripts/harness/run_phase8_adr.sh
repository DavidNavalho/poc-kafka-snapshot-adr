...
run_id="${PHASE8_ADR_RUN_ID:-phase8-adr-$(timestamp_utc)}"
run_root="${ROOT_DIR}/artifacts/phase8-adr-runs/${run_id}"
...
if [[ "${scenario_selector}" == "all" ]]; then
  scenarios=("${PHASE8_ADR_SCENARIOS[@]}")
else
...
for assertion_name in "${PHASE8_ADR_ASSERTION_FIELDS[@]}"; do
...
for scenario in "${scenarios[@]}"; do
  ...
  if [[ "${valid}" -ne 1 ]]; then
    echo "Unknown phase 8 ADR scenario '${scenario}'. Valid: ${PHASE8_ADR_SCENARIOS[*]}" >&2
    exit 1
  fi

  echo "==== Phase 8 ADR scenario: ${scenario} ===="
  "${SCRIPT_DIR}/reset_lab.sh"
  "${SCRIPT_DIR}/start_source.sh"
  "${SCRIPT_DIR}/seed_source_phase8_adr.sh"
  purge_if_low_disk 20
  "${SCRIPT_DIR}/snapshot_copy_phase8_adr.sh" "${scenario}"
  ...
  set +e
  PHASE8_ADR_TARGET_START_RC="${target_start_rc}" "${SCRIPT_DIR}/validate_target_phase8_adr.sh" "${scenario}"
  validate_rc=$?
  set -e
  ...
  status_file="${ROOT_DIR}/artifacts/latest/target/phase8_adr/${scenario}/status.txt"
  summary_env_file="${ROOT_DIR}/artifacts/latest/target/phase8_adr/${scenario}/assertions_summary.env"
  ...
  assertions_file="${ROOT_DIR}/artifacts/latest/target/phase8_adr/${scenario}/assertions.tsv"
...
cp "${summary_file}" "${ROOT_DIR}/artifacts/latest/phase8_adr_summary.tsv"

echo "Phase 8 ADR run summary: ${summary_file}"
if [[ "${overall_rc}" -ne 0 ]]; then
  echo "At least one scenario ended in failed state."
  exit 1
fi
echo "Phase 8 ADR scenarios completed (recovered/degraded states accepted)."
